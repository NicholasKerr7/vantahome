import { AC_TEMP_MAX_C, AC_TEMP_MIN_C, type DeviceMode, useHomeStore } from '../store/useHomeStore';
...
export type Command =
  ...
  | { deviceId: string; op: 'nav'; action: 'up' | 'down' | 'left' | 'right' | 'select' | 'home' };
...
export function applyCommand(cmd: Command) {
  ...
  switch (cmd.op) {
    ...
    case 'set-temp': {
      const value = Math.max(AC_TEMP_MIN_C, Math.min(AC_TEMP_MAX_C, Math.round(cmd.value)));
