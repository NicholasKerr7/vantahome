  ...
  const timer = setInterval(() => {
    ...
    devices.forEach((device) => {
      ...
      if (device.kind === 'energy') {
        const nextPower = clamp((device.powerW ?? 600) + (Math.random() - 0.5) * 160, 200, 1600);
        const nextEnergy = (device.energyTodayKwh ?? 0) + (nextPower / 1000) * deltaHours;
        deviceClient.pushState(device.id, { powerW: Math.round(nextPower), energyTodayKwh: +nextEnergy.toFixed(2) });
      }
      if (device.kind === 'water') {
        const flowBase = Math.random() > 0.7 ? Math.random() * 12 : Math.random() * 3;
        const nextFlow = clamp((device.waterLpm ?? 0) * 0.6 + flowBase, 0, 18);
        const nextToday = (device.waterTodayL ?? 0) + (nextFlow / 60) * (intervalMs / 1000);
        deviceClient.pushState(device.id, { waterLpm: Math.round(nextFlow * 10) / 10, waterTodayL: Math.round(nextToday) });
      }
      if (device.kind === 'air') {
        const nextAqi = clamp((device.airQualityIndex ?? 30) + (Math.random() - 0.5) * 6, 10, 140);
        const nextHumidity = clamp((device.humidity ?? 45) + (Math.random() - 0.5) * 3, 30, 65);
