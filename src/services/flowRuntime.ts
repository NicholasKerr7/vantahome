      ...
      state.devices.forEach((device) => {
        ...
        flows.forEach((flow) => {
          ...
          flow.triggers.forEach((trigger, idx) => {
            ...
            runFlow(flow, flowCooldownMs, running, lastFlowRun, `device-${idx}`);
          });
        });
      });
    }
      ...
      state.household.forEach((member) => {
        ...
        flows.forEach((flow) => {
          ...
          flow.triggers.forEach((trigger, idx) => {
            ...
            runFlow(flow, flowCooldownMs, running, lastFlowRun, `presence-${idx}`);
          });
        });
      });
    }
  ...
  const tick = () => {
    ...
    state.flows.forEach((flow) => {
      ...
      flow.triggers.forEach((trigger, idx) => {
        ...
        if (trigger.hour !== now.getHours() || trigger.minute !== now.getMinutes()) return;
    ...
    state.rules.forEach((rule) => {
      ...
      if (rule.trigger.hour !== now.getHours() || rule.trigger.minute !== now.getMinutes()) return;
...
function snapshotPresence(members: Array<{ id: string; status: 'home' | 'away' }>) {
...
function conditionsPass(conditions: FlowCondition[], now: Date, state: ReturnType<typeof useHomeStore.getState>) {
...
async function executeActions(actions: FlowAction[]) {
  for (const action of actions) {
    if (action.type === 'delay') {
      const seconds = Number.isFinite(action.seconds) ? Math.max(1, action.seconds) : 1;
...
function runRule(rule: AutomationRule) {
  ...
  deviceClient
    .sendCommand({ op: 'toggle', deviceId: rule.action.deviceId, on: rule.action.on })
    .catch(() => {});
}
