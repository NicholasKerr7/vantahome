...
import { AC_TEMP_MAX_C, AC_TEMP_MIN_C, useHomeStore } from '../store/useHomeStore';
        ...
        <View style={{ width: contentWidth, paddingHorizontal: isTablet ? 0 : gutter }}>
          ...
          <View style={[styles.grid, isWide && { flexDirection: 'row', flexWrap: 'wrap', gap: cardGap }]}>
            {rules.map((r) => (
              ...
              >
                <View style={{ flex: 1 }}>
                  <Text style={[styles.name, { fontSize: cardTitleSize }]}>{r.name}</Text>
                  <Text style={[styles.sub, { fontSize: cardSubSize }]}>
                    Trigger: {String(r.trigger.hour).padStart(2, '0')}:{String(r.trigger.minute).padStart(2, '0')} • Action:{' '}
                    {r.action.type === 'set-ac' ? `AC → ${r.action.tempC}°C` : `Toggle → ${r.action.on ? 'ON' : 'OFF'}`}
                  </Text>
                </View>
                <Switch
                  value={r.enabled}
        ...
        <View style={styles.modalOverlay}>
          ...
          <KeyboardAvoidingView behavior={Platform.select({ ios: 'padding', android: undefined })}>
            ...
            >
              ...
              <Text style={[styles.modalLabel, { fontSize: modalLabelSize }]}>Device</Text>
                ...
                {devices.map((d) => {
                  ...
                  return (
                    <Pressable
                      ...
                      style={[
                        styles.devicePill,
                        { height: modalInputHeight, borderRadius: Math.round(modalInputHeight / 2) },
              ...
              <Text style={[styles.modalLabel, { fontSize: modalLabelSize }]}>Time</Text>
              <View style={styles.timeRow}>
                <TextInput
                  ...
                  style={[styles.timeInput, { height: modalInputHeight, borderRadius: Math.round(modalInputHeight * 0.28) }]}
                  maxLength={2}
                />
              ...
              {isAC ? (
                <>
                  <Text style={[styles.modalLabel, { fontSize: modalLabelSize }]}>Temperature</Text>
                  <View style={styles.sliderRow}>
                    <Text style={[styles.sliderValue, { fontSize: modalLabelSize }]}>{temp}°C</Text>
                    <Slider
                      style={{ flex: 1 }}
                ...
                <>
                  <Text style={[styles.modalLabel, { fontSize: modalLabelSize }]}>Action</Text>
                  <View style={styles.toggleRow}>
                    <Pressable
                      style={[
                        styles.toggleBtn,
                        { height: modalInputHeight, borderRadius: Math.round(modalInputHeight * 0.28) },
                        toggleOn && styles.toggleBtnActive,
                      ]}
                      onPress={() => setToggleOn(true)}
                    >
                      <Ionicons name="power" size={Math.round(16 * scale)} color={toggleOn ? '#fff' : 'rgba(12,12,18,0.7)'} />
                      <Text style={[styles.toggleText, { fontSize: modalLabelSize }, toggleOn && styles.toggleTextActive]}>
                        On
                      </Text>
                    </Pressable>
                    <Pressable
              ...
              <View style={styles.modalRow}>
                ...
                <Pressable
                  ...
                  disabled={!canCreate}
                >
                  <Text style={[styles.modalPrimaryText, { fontSize: modalLabelSize }]}>
...
const styles = StyleSheet.create({
  ...
  devicePillActive: { backgroundColor: 'rgba(107,60,255,0.2)', borderColor: 'rgba(107,60,255,0.3)' },
  devicePillText: { color: 'rgba(12,12,18,0.7)', fontWeight: '800', fontSize: 12 },
