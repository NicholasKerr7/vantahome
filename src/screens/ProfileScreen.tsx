...
export default function ProfileScreen({ navigation }: Props) {
  const { contentWidth, gutter, topPad, isTablet, isLandscape, scale } = useResponsive(640);
  ...
  const [avatarColor, setAvatarColor] = useState(profile.avatarColor ?? '#B46BFF');
  ...
  const [newMemberRole, setNewMemberRole] = useState<'Owner' | 'Admin' | 'Guest'>('Guest');
  ...
  const pickAvatar = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission needed', 'Enable photo access to upload a profile avatar.');
      return;
    }
  ...
  const pickHouseholdAvatar = async () => {
    ...
    const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (status !== 'granted') {
      Alert.alert('Permission needed', 'Enable photo access to add a household member.');
      return;
    }
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images,
        ...
        <View style={{ width: contentWidth }}>
          ...
          <View style={[styles.card, { padding: cardPad, borderRadius: cardRadius }]}>
            <View style={styles.avatarRow}>
              <AvatarChip name={name || 'Vanta Home'} size={avatarSize} color={avatarColor} uri={avatarUri} />
              <View style={{ flex: 1 }}>
                ...
                <View style={styles.avatarActions}>
                  <Pressable style={styles.avatarBtn} onPress={pickAvatar}>
                    <Ionicons name="cloud-upload-outline" size={Math.round(16 * scale)} color={theme.colors.text} />
                    <Text style={[styles.avatarBtnText, { fontSize: hintSize }]}>Upload photo</Text>
                  </Pressable>
                  {avatarUri ? (
                    <Pressable style={[styles.avatarBtn, styles.avatarBtnGhost]} onPress={() => setAvatarUri('')}>
                      <Ionicons name="trash-outline" size={Math.round(16 * scale)} color={theme.colors.subtext} />
                      <Text style={[styles.avatarBtnText, { fontSize: hintSize }]}>Remove</Text>
                    </Pressable>
                  ) : null}
                </View>
                <View style={styles.swatchRow}>
                  {['#B46BFF', '#7A5CFF', '#FF9AA2', '#A0E9FF', '#FFD166', '#A5FF9B'].map((c) => (
                    <Pressable
                      key={c}
                      ...
                      style={[
                        styles.swatch,
                        {
                          ...
                          borderRadius: Math.round((isTablet ? 12 : 10) * scale),
                          backgroundColor: c,
                        },
            ...
            <View style={[styles.formGrid, isWide && { flexDirection: 'row', gap: gridGap }]}>
              <View style={[styles.formColumn, isWide && { flex: 1 }]}>
                <Text style={[styles.label, { fontSize: labelSize }]}>Name</Text>
                <TextInput
                  value={name}
              ...
              <View style={[styles.formColumn, isWide && { flex: 1 }]}>
                <Text style={[styles.label, { fontSize: labelSize }]}>Phone</Text>
                <TextInput
                  value={phone}
                ...
                <Text style={[styles.label, { fontSize: labelSize }]}>Home name</Text>
                <TextInput
                  value={homeName}
          ...
          <View style={[styles.card, { padding: cardPad, borderRadius: cardRadius }]}>
            <Text style={[styles.sectionTitle, { fontSize: labelSize }]}>Personalization</Text>
            <Text style={[styles.cardHint, { fontSize: hintSize }]}>Temperature unit</Text>
            <View style={styles.chipRow}>
              {(['C', 'F'] as const).map((unit) => {
                ...
                return (
                  ...
                  >
                    <Text style={[styles.chipText, active && styles.chipTextActive]}>{unit === 'C' ? 'Celsius' : 'Fahrenheit'}</Text>
                  </Pressable>
                );
              })}
            </View>

            <Text style={[styles.cardHint, { fontSize: hintSize, marginTop: 10 }]}>Time format</Text>
            <View style={styles.chipRow}>
            ...
            <Text style={[styles.cardHint, { fontSize: hintSize, marginTop: 10 }]}>Timezone</Text>
            <TextInput
              value={timezone}
          ...
          <View style={[styles.card, { padding: cardPad, borderRadius: cardRadius }]}>
            <Text style={[styles.sectionTitle, { fontSize: labelSize }]}>Home overview</Text>
            <View style={styles.row}>
              <Text style={[styles.rowLabel, { fontSize: labelSize }]}>Home</Text>
              <Text style={[styles.rowValue, { fontSize: labelSize }]}>{homeName || profile.homeName || 'Vanta Home'}</Text>
            </View>
            <View style={styles.row}>
              <Text style={[styles.rowLabel, { fontSize: labelSize }]}>Rooms</Text>
              <Text style={[styles.rowValue, { fontSize: labelSize }]}>{roomsCount}</Text>
            </View>
            <View style={styles.row}>
              <Text style={[styles.rowLabel, { fontSize: labelSize }]}>Devices</Text>
              <Text style={[styles.rowValue, { fontSize: labelSize }]}>{devicesCount}</Text>
            </View>
          </View>

          <View style={[styles.card, { padding: cardPad, borderRadius: cardRadius }]}>
            <Text style={[styles.sectionTitle, { fontSize: labelSize }]}>Quick preferences</Text>
            <View style={styles.row}>
              <Text style={[styles.rowLabel, { fontSize: labelSize }]}>Haptics</Text>
              <Switch
                value={prefs.haptics}
            ...
            <View style={styles.row}>
              <Text style={[styles.rowLabel, { fontSize: labelSize }]}>Notifications</Text>
              <Switch
          ...
          <View style={[styles.card, { padding: cardPad, borderRadius: cardRadius }]}>
            <Text style={[styles.sectionTitle, { fontSize: labelSize }]}>Household</Text>
            {household.map((member) => (
              <View key={member.id} style={styles.memberRow}>
                ...
                />
                <View style={{ flex: 1 }}>
                  <Text style={[styles.memberName, { fontSize: labelSize }]}>{member.name}</Text>
                  <Text style={[styles.memberRole, { fontSize: hintSize }]}>{member.role}</Text>
                </View>
                <View style={styles.memberBadge}>
                  <Text style={[styles.memberBadgeText, { fontSize: hintSize }]}>
                ...
                <Pressable
                  ...
                  hitSlop={8}
                >
                  <Ionicons name="close" size={Math.round(14 * scale)} color={theme.colors.subtext} />
                </Pressable>
              </View>
            ))}

            <View style={styles.addMemberCard}>
              <Text style={[styles.cardHint, { fontSize: hintSize }]}>Add person</Text>
              <TextInput
                value={newMemberName}
              ...
              <View style={styles.avatarActions}>
                <Pressable style={styles.avatarBtn} onPress={pickHouseholdAvatar}>
                  <Ionicons name="image-outline" size={Math.round(16 * scale)} color={theme.colors.text} />
                  <Text style={[styles.avatarBtnText, { fontSize: hintSize }]}>
                    {newMemberAvatar ? 'Change photo' : 'Add photo'}
                  </Text>
                </Pressable>
                {newMemberAvatar ? (
                  <Pressable style={[styles.avatarBtn, styles.avatarBtnGhost]} onPress={() => setNewMemberAvatar('')}>
                    <Ionicons name="close" size={Math.round(16 * scale)} color={theme.colors.subtext} />
                    <Text style={[styles.avatarBtnText, { fontSize: hintSize }]}>Remove</Text>
                  </Pressable>
                ) : null}
              </View>
              <Pressable
                style={[styles.secondaryBtn, !newMemberName.trim() && { opacity: 0.6 }]}
                onPress={handleAddMember}
                disabled={!newMemberName.trim()}
              >
                <Ionicons name="person-add" size={Math.round(16 * scale)} color={theme.colors.text} />
                <Text style={[styles.secondaryBtnText, { fontSize: labelSize }]}>Add person</Text>
              </Pressable>
            </View>
          </View>
          ...
          <Pressable
            ...
            disabled={!name.trim()}
          >
            <Text style={[styles.saveText, { fontSize: labelSize }]}>Save profile</Text>
          </Pressable>
        </View>
      </ScrollView>
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  avatarRow: { flexDirection: 'row', gap: 16, alignItems: 'center', marginBottom: 12 },
  ...
  sectionTitle: { color: theme.colors.text, fontWeight: '900', marginBottom: 10 },
  ...
  chipActive: { backgroundColor: 'rgba(180,107,255,0.32)', borderColor: 'rgba(180,107,255,0.45)' },
