...
function StepBtn({
  ...
  return (
    <Pressable
      ...
      style={[styles.stepBtn, { width: size, height: size, borderRadius: Math.round(size * 0.4) }]}
      hitSlop={6}
    >
...
function ControlPill({
  ...
  return (
    <Pressable
      onPress={(e) => stop(e, onPress)}
      style={[styles.controlPill, { height, borderRadius: Math.round(height * 0.44) }]}
      hitSlop={6}
    >
...
export default function DeviceTile({
  ...
  const minHeight = Math.round((isTablet ? (isLandscape ? 190 : 210) : 168) * scale);
  ...
  const meta =
    device.kind === 'ac'
      ...
      : device.kind === 'fridge'
      ? `${device.tempC ?? 4}°C`
      : device.kind === 'garage' || device.kind === 'door' || device.kind === 'gate' || device.kind === 'window'
  ...
  return (
    <Pressable
      style={[styles.card, { padding: cardPad, borderRadius: cardRadius, minHeight }]}
      onPress={onPress}
    ...
    >
      <View style={styles.topRow}>
        ...
        <Pressable
          ...
          hitSlop={6}
        >
          <Ionicons name="power" size={powerIconSize} color={theme.colors.text} />
        </Pressable>
      </View>
      ...
      <View style={[styles.divider, { marginTop: dividerGap, marginBottom: dividerGap }]} />
      ...
      {device.kind === 'light' && (
        <View style={styles.controlsRow}>
          ...
          />
          <Text style={[styles.controlValue, { fontSize: controlFont }]}>{device.brightness ?? 60}%</Text>
          <StepBtn
            icon="add"
            size={stepSize}
            iconSize={stepIconSize}
      ...
      {device.kind === 'ac' && (
        <View style={styles.controlsRow}>
          <StepBtn
            ...
            onPress={() =>
              sendPatch({
                tempC: clamp((device.tempC ?? 22) - 1, AC_TEMP_MIN_C, AC_TEMP_MAX_C),
                isOn: true,
              })
            }
          />
          <Text style={[styles.controlValue, { fontSize: controlFont }]}>{device.tempC ?? 22}°C</Text>
          <StepBtn
            icon="add"
            size={stepSize}
            iconSize={stepIconSize}
      ...
      {device.kind === 'tv' && (
        <View style={styles.controlsRow}>
          <StepBtn
            ...
            size={stepSize}
            iconSize={stepIconSize}
            onPress={() => sendPatch({ volume: clamp((device.volume ?? 20) - 5, 0, 100), isOn: true })}
          />
          <Text style={[styles.controlValue, { fontSize: controlFont }]}>Vol {device.volume ?? 20}</Text>
          <StepBtn
            icon="volume-high"
            size={stepSize}
            iconSize={stepIconSize}
      ...
      {device.kind === 'fan' && (
        <View style={styles.controlsRow}>
          <StepBtn
            ...
            iconSize={stepIconSize}
            onPress={() =>
              sendPatch({ speed: clamp((device.speed ?? 50) - 10, 0, 100), isOn: true })
            }
          />
          <Text style={[styles.controlValue, { fontSize: controlFont }]}>Speed {device.speed ?? 50}%</Text>
          <StepBtn
            icon="add"
            size={stepSize}
            iconSize={stepIconSize}
      ...
      {device.kind === 'speaker' && (
        <View style={styles.controlsRow}>
          <StepBtn
            ...
            iconSize={stepIconSize}
            onPress={() =>
              sendPatch({ volume: clamp((device.volume ?? 20) - 5, 0, 100), isOn: true })
            }
          />
          <Text style={[styles.controlValue, { fontSize: controlFont }]}>Vol {device.volume ?? 20}</Text>
          <StepBtn
      ...
      {(device.kind === 'door' || device.kind === 'garage' || device.kind === 'gate' || device.kind === 'window') && (
...
const styles = StyleSheet.create({
  ...
  meta: { marginTop: 6, color: theme.colors.subtext, fontWeight: '700', fontSize: 12 },

  divider: { height: 1, backgroundColor: 'rgba(255,255,255,0.10)', marginTop: 12, marginBottom: 12 },
  ...
  controlValue: { flex: 1, textAlign: 'center', color: theme.colors.text, fontWeight: '900' },
  ...
  controlPillText: { color: theme.colors.text, fontWeight: '900', fontSize: 12 },
});
