...
const devices: Device[] = [
  { id: 'd1', name: 'AC', kind: 'ac', roomId: 'r1', isOn: true, tempC: 22, mode: 'cold' },
  { id: 'd2', name: 'Light', kind: 'light', roomId: 'r1', isOn: true, brightness: 70 },
  { id: 'd3', name: 'TV', kind: 'tv', roomId: 'r2', isOn: false, volume: 30 },
];

describe('RoomCarousel', () => {
  it('snaps between cards (swipe carousel)', () => {
    const tree = renderer.create(<RoomCarousel rooms={rooms} devices={devices} onRoomPress={() => undefined} />);
  ...
  it('renders stacked layers for the active card', () => {
    const tree = renderer.create(<RoomCarousel rooms={rooms} devices={devices} />);
