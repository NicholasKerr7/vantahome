...
const ACTIVE_GRADIENT = ['#FFFFFF', 'rgba(248,240,255,0.98)', 'rgba(242,233,255,0.94)'];
...
function RoomCard({
  ...
  const roomDevices = useMemo(() => devices.filter((d) => d.roomId === item.id), [devices, item.id]);

  const allDevices = isWholeHome ? devices : roomDevices;
  const running = allDevices.filter((d) => d.isOn).length;

  const displayDevices = isWholeHome ? wholeHomeDevices ?? allDevices : roomDevices;
  ...
  const animStyle = useAnimatedStyle(() => {
    ...
    const scale = interpolate(Math.abs(dist), [0, 1], [1, layout.inactiveScale], Extrapolation.CLAMP);
    const lift = interpolate(Math.abs(dist), [0, 1], [0, layout.inactiveLift], Extrapolation.CLAMP);
    const opacity = interpolate(Math.abs(dist), [0, 1], [1, layout.inactiveOpacity], Extrapolation.CLAMP);

    return { transform: [{ translateY: lift }, { scale }], opacity: isActive ? 1 : opacity };
  ...
  const cardGradient = isActive
    ...
    : isTablet
      ...
      : STACKED_GRADIENT;
  const borderColor = isTablet ? 'rgba(140,180,255,0.55)' : 'rgba(255,255,255,0.72)';
  ...
  return (
    <Animated.View
      style={[
        styles.item,
        { width: layout.cardWidth, height: layout.cardHeight + layout.stackGap },
        animStyle,
      ]}
    >
      ...
      {isActive && (
        <>
          ...
          >
            {stackTitles?.[1] ? (
              <Text style={[styles.stackTitle, { fontSize: layout.stackTitleSize }]} numberOfLines={1}>
                {stackTitles[1]}
              </Text>
            ) : null}
          </View>
      ...
      >
        ...
        >
          <Text style={[styles.title, { fontSize: titleSize }, !isActive && styles.titleInactive]} numberOfLines={1}>
            {item.name}
          </Text>
          {isActive ? (
                ...
                <View style={[styles.iconTile, { width: layout.iconTileWidth }]}>
                  <View
                    style={[
                      styles.iconBubble,
                      styles.moreBubble,
                      { width: layout.bubbleSize, height: layout.bubbleSize, borderRadius: layout.bubbleRadius },
                    ]}
                  >
                    <Text style={styles.moreCount}>+{remaining}</Text>
                  </View>
                  <Text style={[styles.iconLabel, { fontSize: layout.iconLabelSize }]}>More</Text>
                </View>
              )}
            </View>
          ) : null}
...
export default function RoomCarousel({
  ...
  const cardWidth = Math.max(minCard, Math.round(listWidth - sidePad * 2 - gap));
  ...
  const onMomentumEnd = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    updateIndex(Math.max(0, Math.round(e.nativeEvent.contentOffset.x / itemWidth)));
  };
    ...
    <View style={styles.wrap}>
      <Animated.FlatList
        ...
        renderItem={({ item, index }) => {
          const stackTitles =
            index === activeIndex ? [data[index + 1]?.name, data[index + 2]?.name] : undefined;
          return (
            <RoomCard
              item={item}
              index={index}
