...
const DeviceBottomSheet = forwardRef<BottomSheetModal, Props>(function DeviceBottomSheet(
  { device, onClose, onOpenDetails, onGoToAutomations, onToggle, onQuickSchedule, onDelete },
  ref
) {
  const { contentWidth, gutter, isTablet, isLandscape, scale } = useResponsive(760);
  // Two-stage sheet: compact quick view, then expanded controls.
  const snapPoints = useMemo(
    () => (isTablet ? (isLandscape ? ['40%', '72%'] : ['42%', '74%']) : ['38%', '68%']),
    ...
    >
      <BottomSheetView style={[styles.sheetWrap, { padding: gutter }]}>
        <View
          style={[
            styles.glass,
            { width: '100%', maxWidth: contentWidth, alignSelf: 'center', borderRadius: glassRadius },
          ]}
        >
          {/* Real blur glass */}
            ...
            {!device ? null : (
              <>
                <View style={styles.header}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.title, { fontSize: titleSize }]}>{device.name}</Text>
                ...
                <View style={[styles.section, { padding: sectionPad, borderRadius: Math.round(sectionPad * 1.4) }]}>
                  <Text style={[styles.sectionTitle, { fontSize: sectionTitleSize }]}>Quick controls</Text>
                ...
                <View style={[styles.section, { padding: sectionPad, borderRadius: Math.round(sectionPad * 1.4) }]}>
                  <Text style={[styles.sectionTitle, { fontSize: sectionTitleSize }]}>Schedule</Text>
                  <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
                    <Pressable
                      style={[styles.chip, { height: chipHeight, borderRadius: chipRadius }]}
                      onPress={() => {
                        haptic();
                        onQuickSchedule({ hour: 21, minute: 0 });
                      }}
                    >
                      <Text style={[styles.chipText, { fontSize: chipText }]}>Tonight 9:00 PM</Text>
                    </Pressable>
                    ...
                    >
                      <Text style={[styles.chipText, { fontSize: chipText }]}>Tomorrow 7:00 AM</Text>
                    </Pressable>
                  </View>

                  <Pressable
                    style={[styles.linkBtn, { marginTop: 12, height: linkHeight, borderRadius: Math.round(linkHeight * 0.4) }]}
                    onPress={onGoToAutomations}
                  >
                    <Ionicons name="flash" size={Math.round((isTablet ? 18 : 16) * scale)} color={theme.colors.text} />
                    <Text style={[styles.linkBtnText, { fontSize: chipText }]}>
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 6 }}>
                  <Pressable
                    style={[
                      styles.footerBtn,
                      { height: footerHeight, borderRadius: Math.round(footerHeight * 0.38) },
                    ]}
                    onPress={onOpenDetails}
                  >
                    <Text style={[styles.footerBtnText, { fontSize: chipText }]}>Open details</Text>
                  </Pressable>
                  <Pressable
                    style={[
                      styles.footerBtn,
                    ...
                    onPress={onClose}
                  >
                    <Text style={[styles.footerBtnText, { fontSize: chipText }]}>Close</Text>
                  </Pressable>
                </View>
                {onDelete ? (
                  <Pressable
                  ...
                  >
                    <Text style={[styles.footerBtnText, { color: '#ffebef', fontSize: chipText }]}>
                      Delete device
                    </Text>
                  </Pressable>
                ) : null}
