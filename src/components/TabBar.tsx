...
export default function TabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  const barHeight = Math.round((isTablet ? (isLandscape ? 74 : 72) : 68) * scale);
        ...
        return (
          ...
          >
            <View
              style={[
                styles.iconWrap,
                { width: iconWrapSize, height: iconWrapSize, borderRadius: iconRadius },
                isFocused && styles.iconWrapFocused,
              ]}
            >
              <Ionicons name={icon} size={iconSize} color={isFocused ? theme.colors.text : theme.colors.subtext} />
            </View>
          </Pressable>
        );
      })}
