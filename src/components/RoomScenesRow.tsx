  ...
  return (
    <View style={{ marginTop: 14 }}>
      <View style={[styles.header, { paddingHorizontal: gutter }]}>
        ...
        <View style={[styles.hChip, { height: chipHeight, borderRadius: chipRadius, paddingHorizontal: headerGap * 2 }]}>
          <Ionicons name="sparkles" size={Math.round((isTablet ? 16 : 14) * scale)} color={theme.colors.text} />
          <Text style={[styles.hChipText, { fontSize: pillText }]}>One tap</Text>
        </View>
      </View>
      ...
      <ScrollView
        ...
        contentContainerStyle={[styles.row, { paddingHorizontal: gutter, gap: rowGap }]}
      >
        {scenes.map((s) => (
          <Pressable
            key={s.id}
            style={[
              styles.pill,
              { height: pillHeight, borderRadius: pillRadius, paddingHorizontal: Math.round(pillHeight * 0.4) },
              s.id === activeSceneId && styles.pillActive,
            ]}
            onPress={() => onRun(s.id)}
          >
            <Text style={[styles.pillText, { fontSize: pillText }, s.id === activeSceneId && styles.pillTextActive]}>
              {s.name}
            </Text>
          </Pressable>
        ))}
