...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const updates = [] as Array<{ device_id: string; state: Record<string, unknown> }>;
    ...
    for (const evt of events) {
      ...
      updates.push({ device_id: deviceId, state: { ...(existing?.state ?? {}), ...evt.state } });
    }

    if (!updates.length) {
      return new Response(JSON.stringify({ error: 'No valid events provided.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
