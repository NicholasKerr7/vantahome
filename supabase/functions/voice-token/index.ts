...
import { getVoiceClient, randomToken, verifyClientSecret } from '../_shared/voiceAuth.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (grantType === 'authorization_code') {
      ...
      if (!code || !redirectUri) return jsonResponse({ error: 'invalid_request' }, 400);
      ...
      const { error: insertError } = await admin.from('voice_oauth_tokens').insert({
    ...
    return jsonResponse({ error: (err as Error).message ?? 'server_error' }, 500);
  }
});
