...
import { googleDeviceType, googleTraits, googleState } from '../_shared/voiceMappings.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (intent === 'action.devices.SYNC') {
      ...
      return new Response(
        JSON.stringify({ requestId, payload: { agentUserId: userId, devices: payloadDevices } }),
    ...
    if (intent === 'action.devices.QUERY') {
      ...
      return new Response(JSON.stringify({ requestId, payload: { devices: response } }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (intent === 'action.devices.EXECUTE') {
      ...
      for (const cmd of commands) {
        ...
        for (const deviceId of deviceIds) {
          ...
          if (!device) {
            results.push({ ids: [deviceId], status: 'ERROR', errorCode: 'deviceNotFound' });
            continue;
          }
          ...
          execs.forEach((exec: any) => {
            ...
            if (exec.command === 'action.devices.commands.ThermostatTemperatureSetpoint') {
              const val = Number(exec.params?.thermostatTemperatureSetpoint ?? 22);
              patch.tempC = val;
            }
          });
          ...
          if (Object.keys(patch).length) {
            await enqueueDeviceCommand(deviceId, { source: 'google', patch, execs });
          }
      ...
      return new Response(JSON.stringify({ requestId, payload: { commands: results } }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
