...
wss.on('connection', (ws) => {
  ...
  if (snapshot.length) {
    ws.send(JSON.stringify({ type: 'snapshot', devices: snapshot, ts: Date.now() }));
  }
  ...
  ws.on('message', (raw) => {
    ...
    if (data?.type === 'state' && data.deviceId && data.patch) {
      ...
      broadcast({ type: 'state', deviceId: data.deviceId, patch: next, ts: Date.now() });
      return;
    }
    ...
    if (data?.type === 'command' && data.payload?.deviceId) {
      ...
      broadcast({ type: 'state', deviceId: data.payload.deviceId, patch: next, ts: Date.now() });
    }
  });
});
